...
waitUntil {
	sleep 1; 
	((_plane distance2D _position) <= 900)
};
...
waitUntil {
	sleep 1; 
	(_plane distance2D _position) <= 800
};
...
["RS_INV_fnc_Server_PlayerAirDrop", 4, (format ["Players in Array (%1), Array Size (%2)", _playerArray, (_countPArray + 1)]), false] call RS_fnc_LoggingHelper;
...
if (_countPArray != -1) then
{
	...
	["RS_INV_fnc_Server_PlayerAirDrop", 4, (format ["Players (%1), Executing toJStand", (_playerArray select 0)]), false] call RS_fnc_LoggingHelper;
};
...
for "_i" from 1 to _countPArray do
{
	...
	["RS_INV_fnc_Server_PlayerAirDrop", 4, (format ["Players (%1), Executing toStand", (_playerArray select _i)]), false] call RS_fnc_LoggingHelper;
};

// Waits until the plane makes it to the drop zone
waitUntil {
	sleep 1; 
	((_plane distance2D _position) <= 400)
};
...
while { _countPArray >= 0 } do
{
	...
	["RS_INV_fnc_Server_PlayerAirDrop", 4, (format ["Players (%1), Executing toDoor", (_playerArray select _countPArray)]), false] call RS_fnc_LoggingHelper;
